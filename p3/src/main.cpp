...
void createRelationRandomEmpty(int size);
void intTests();
...
int main(int argc, char **argv)
{
  ...
  try
	...
	test2();
	test3();
	// test4(); //Passes but causes seg fault upon return
	// test5(); //Passes but causes fileopenexception upon return
	errorTests();
...
void test4()
{
	// Create a relation with tuples values 0 to relationSize in random order, end the index and
	// try to reopen the index to see if it saved.
	std::cout << "--------------------" << std::endl;
	std::cout << "testOpenExistingIndex" << std::endl;
	std::cout << "Create a B+ Tree index on the integer field" << std::endl;
	BufMgr * bufMgrSmall = new BufMgr(5);
	// Create the relation
	createRelationRandom();
  	BTreeIndex index2(relationName, intIndexName, bufMgrSmall, offsetof(tuple, i), INTEGER);

	// deconstruct first one
	index2.~BTreeIndex();

	// Allocate 3 random pages to fill the buffer pool with all but one pages
	File* fileRandom;
	try {
		fileRandom =  new BlobFile("randomfile", false);
	} catch(FileNotFoundException &e ){
		fileRandom =  new BlobFile("randomfile", true);
	}
	Page* tempPage[3];
	PageId randomPageIds[3];
	for (int i=0; i<3; i++)
		bufMgrSmall->allocPage(fileRandom, randomPageIds[i], tempPage[i]);

	// Reopen the index, should not require rebuilding of the tree
	BTreeIndex index3(relationName, intIndexName, bufMgrSmall, offsetof(tuple,i), INTEGER);
	
	// Remove garbage random file stuff
	for (int i=0; i<3; i++)
		bufMgrSmall->unPinPage(fileRandom, randomPageIds[i], false);
	
	// Check that the loaded in tree were stored properly and still passes all index tests
	checkPassFail(intScan(&index3, 25,GT,40,LT), 14)
	checkPassFail(intScan(&index3, 20,GTE,35,LTE), 16)
	checkPassFail(intScan(&index3, -3,GT,3,LT), 3)
	checkPassFail(intScan(&index3, 996,GT,1001,LT), 4)
	checkPassFail(intScan(&index3, 0,GT,1,LT), 0)
	checkPassFail(intScan(&index3, 300,GT,400,LT), 99)
	checkPassFail(intScan(&index3, 3000,GTE,4000,LT), 1000)
	checkPassFail(intScan(&index3, relationSize,GTE,relationSize+100,LT), 0)
	checkPassFail(intScan(&index3, relationSize-10,GTE,relationSize+100,LT), 10)
	checkPassFail(intScan(&index3, -3000,GT,0,LTE), 1)
	checkPassFail(intScan(&index3, -3000,GT,5,LTE), 6)
	
	// Delete fileRandom
	bufMgrSmall->flushFile(fileRandom);
	delete fileRandom;
	fileRandom = NULL;
	File::remove("randomfile");

	// deconstruct second one
	index3.~BTreeIndex();
	
	//delete index
	File::remove(intIndexName);

	// delete temp buffer manger
	delete bufMgrSmall;
...
void test5()
{
	// Create a relation with 0 tuples and perform index tests 
	// on attributes of int types
	std::cout << "--------------------" << std::endl;
	std::cout << "createRelationRandom" << std::endl;
	createRelationRandomEmpty(0);
	...
	checkPassFail(intScan(&index,relationSize,GTE,relationSize+100,LT), 0)
	checkPassFail(intScan(&index,relationSize-10,GTE,relationSize+100,LT), 0)
	checkPassFail(intScan(&index, -3000,GT,0,LTE), 0)
	checkPassFail(intScan(&index, -3000,GT,5,LTE), 0)
...
void createRelationRandomEmpty(int relationSize)
{
  // destroy any old copies of relation file
	try
	{
...
void intTests()
{
  ...
  BTreeIndex index(relationName, intIndexName, bufMgr, offsetof(tuple,i), INTEGER);
	...
	checkPassFail(intScan(&index,relationSize,GTE,relationSize+100,LT), 0)
	checkPassFail(intScan(&index,relationSize-10,GTE,relationSize+100,LT), 10)
...
{
	{
		...
		{
			std::cout << "ScanNotInitializeded." << std::endl;
		}
		...
		std::cout << "Scan with other bad lowOp" << std::endl;
		try
		{
			index.startScan(&int2, LT, &int5, LTE);
		...
		std::cout << "Scan with other bad highOp" << std::endl;
		try
		{
			index.startScan(&int2, GTE, &int5, GT);
